...
const App: React.FC = () => {
  ...
  const handleDeepAnalysis = async () => {
    if (metrics.length === 0 || isDeepAnalyzing) return;
    setIsDeepAnalyzing(true);
    addLog('Requesting Gemini Pro Deep Structural Analysis...', 'info');
    const result = await deepEngineAnalysis(metrics, config);
    setAdvice(result);
  ...
  const handleOptimizing = async () => {
    ...
    setStatus(EngineStatus.OPTIMIZING);
    addLog('Initiating Neural Core Optimization...', 'info');
    setTimeout(() => {
      setConfig(prev => ({
        ...prev,
        coolantFlow: Math.min(200, prev.coolantFlow + 20),
        plasmaDensity: Math.max(0.5, prev.plasmaDensity - 0.05)
      }));
      setStatus(EngineStatus.RUNNING);
        ...
        <div className="fixed inset-0 z-[100] bg-slate-950 flex flex-col">
          <div className="absolute top-8 right-8 z-[110]">
            <button 
              ...
              className="px-6 py-2 bg-red-600/20 border border-red-500/50 text-red-400 rounded-lg font-black uppercase tracking-widest hover:bg-red-600/40 transition-all flex items-center space-x-2"
            >
